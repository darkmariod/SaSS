...
const paymentQrUrl = (shop) => {
    if (!shop.bank_qr_image) {
        return null;
    }
    return `/storage/${shop.bank_qr_image}`;
};
</script>
        ...
        <div class="space-y-8">
            ...
            <div class="border-t border-gray-200 pt-8">
                ...
                <div class="mt-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:mt-6 sm:p-6">
                    <div class="space-y-4">
                        ...
                        >
                            <input
                                ...
                                :checked="wizard.paymentOption.value === 'at_appointment'"
                                @change="wizard.paymentOption.value = 'at_appointment'"
                            ...
                            <div class="flex-1">
                                ...
                                <p class="mt-1 text-sm text-gray-500 sm:text-base">
                                    Pagás en efectivo o con tarjeta directamente en la barbería.
                                </p>
                            </div>
                            <div class="shrink-0 text-right">
                                <p class="text-sm font-black text-gray-950">USD {{ Number(wizard.total.value).toFixed(2) }}</p>
                                <p class="text-xs text-gray-500">en la cita</p>
                            </div>
                        </label>
                        ...
                        >
                            <input
                                ...
                                name="paymentOption"
                                value="today"
                                :checked="wizard.paymentOption.value === 'today'"
                                @change="wizard.paymentOption.value = 'today'"
                                class="mt-1 h-4 w-4 shrink-0 accent-gray-950"
                            />
                            <div class="flex-1">
                                <div class="flex items-center gap-2">
                                ...
                                <p class="mt-1 text-sm text-gray-500 sm:text-base">
                                    Transferís el total ahora y la cita se confirma más rápido.
                                </p>
                            </div>
                            <div class="shrink-0 text-right">
                                <p class="text-sm font-black text-gray-950">USD {{ Number(wizard.total.value).toFixed(2) }}</p>
                    ...
                    <div
                        v-if="wizard.payToday.value && (shop.bank_name || shop.bank_account || shop.bank_account_owner || shop.payment_instructions || shop.bank_qr_image)"
                    ...
                    >
                        Esta barbería todavía no configuró datos bancarios. Podés crear la reserva y coordinar el pago directamente con el negocio.
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl bg-gray-50 p-4 text-sm text-gray-500 sm:p-5 sm:text-base">
                <template v-if="wizard.payToday.value">
                    Después de reservar, podrás subir el comprobante de transferencia para que la barbería revise y confirme tu cita.
                </template>
                <template v-else>
                    No necesitás hacer ningún pago ahora. Abonás el total al llegar a la barbería.
                </template>
            </div>
